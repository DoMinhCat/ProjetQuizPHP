...
if (isset($_SESSION['id']) && !empty($_SESSION['id'])) {
    ...
    try {
        ...
        if ($success) {
            session_unset();
            session_destroy();
            header("Location: index.php?succes=" . urlencode('Compte supprimé'));
            exit();
        } else {
            header("Location: account.php?error=" . urlencode('Erreur lors de la suppresion'));
            exit();
        }
    } catch (Exception $e) {
        header("Location: account.php?error=" . urlencode('Erreur lors de la suppresion') . '&message=' . urlencode($e->getMessage()));
        exit();
    }
} else {
    header("Location: index.php?error=" . urldecode('Requête invalide'));
    exit();
}
